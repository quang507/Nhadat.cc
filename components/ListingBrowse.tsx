    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {listings.length === 0 && (
        <div className="mt-10 rounded-king border border-line bg-white p-8 text-center">
          ...
          <a
            ...
            className="mt-4 inline-block rounded-full bg-zalo px-5 py-2.5 font-semibold text-white"
          >
            Hỏi qua Zalo
          </a>
        </div>
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-2 text-sm">
          {page > 1 && (
            <Link href={pageUrl(page - 1)} className="rounded-lg border border-line px-4 py-2 hover:border-brand">
              ← Trước
            </Link>
          )}
          <span className="px-3 text-mute">Trang {page}/{totalPages}</span>
          {page < totalPages && (
            <Link href={pageUrl(page + 1)} className="rounded-lg border border-line px-4 py-2 hover:border-brand">
              Sau →
            </Link>
