...
| FR-152 | UF-09 | — | `bot_errors`/`bot_health`, `beat()`, `bot_health_tick()` + cron 15', `log_loi()`/`ghiLoi()`, web `instrumentation.ts`, `/admin` sức khoẻ bot; (e) `canh_bao_ngoai()` → ntfy (`20260904a`), `bot/bridge-zca/VPS.md` | TS-HEALTH, TS-LOG |
...
| FR-167 | UF-01, UF-03, UF-04, UF-05 | WF-02, WF-03 | `20260829d`: policy `listing_media_doc_cong_khai` theo tin lên kệ, `listing_photos_v` cùng điều kiện, revoke EXECUTE hàm nội bộ, ghim `search_path`; `x-bridge-secret` ở mọi function; còn hở OPEN-33 | TS-SEC2 |
