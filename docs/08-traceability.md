...
| FR-177 | UF-04 | — | ⏳ chưa code (07/09): migration `required_facts.priority` (nhóm cơ bản loại/vị trí/diện tích/giá = 1, chuỗi còn lại sau; bỏ hướng/quy hoạch/năm xây khỏi vòng đầu), chọn câu kế ưu tiên cùng nhóm với fact vừa ghi, `khop-cau-tra-loi.ts` mở rộng `chuyenSang` cho mọi khoá + fact `bo_sung` cho câu không nhận ra (FR-177 e), hàm điểm tin 7 tiêu chí (tiền định), `SELLER_SCRIPT_RULES` khích lệ + bản nháp tin, `listings_autopublish` theo điểm ≥ 70; e2e tái hiện 20 câu Gemini lượt 3 + ca "hỏi diện tích, trả lời pháp lý → ghi phap_ly, giữ câu diện tích". Nguồn gốc: `Kịch bản huấn luyện môi giới (Gemini 07-09-2026).md` | — |
| FR-97 | | — | — | — |
