...
| FR-186 | UF-10 | — | `20260909h`: `required_facts.deal`, index (property_type, fact_key, coalesce(deal)), reseed 94 dòng 8 loại, `listing_missing_facts` lọc deal, `nhan_fact` khoá mới; `prompts.ts`: `CAU_HOI_MAU` khoá `fact@loai` + 7 câu mới, `cauHoiMau(…, loai)`, `FACT_LABELS`; `khop-cau-tra-loi.ts`: `HOI_SO`/`TU_KHOA`/`NHAN_HOI_LAI`/`nhanDienFact`/`LIEN_QUAN`; `chat-reply` truyền `property_type` vào `cauHoiMau` (3 chỗ), select `property_type/project_id/area_m2` trong câu chờ; `SELLER_SCRIPT_RULES` thứ tự theo loại; mock `missingFacts()` theo loại + deal. Kiểm: `fr177` 10 ca nhận diện + 5 ca `chonCauKe` + 4 câu mẫu; e2e H4b, N6, N6b, N7, N7b, N8, N8b (TS-KYGUI-30) | — |
| FR-97 | | — | — | — |
