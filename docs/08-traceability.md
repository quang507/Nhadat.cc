...
| FR-203 | UF-11 | — | bảng `bridge_dang_nhap` + `yeu_cau_quet_lai_zalo()` (`20260911b`); `escalation-feed` action `qr` + cờ `quet_lai`; bridge `baoDangNhap()`/`phienChet()`/`actions.retry()`; `/admin` ô `TheZaloClone`; (f) thẻ *Trợ lý AI Bot* tính `botLoi` (bridge im / Zalo chưa đăng nhập / model hết số dư không dự phòng), ô QR lên đầu trang khi `zaloChet`, `AdminShell` chấm đỏ tab *Vận hành*. Kiểm: TS-QR-01 | — |
