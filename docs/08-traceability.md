...
| FR-153 | UF-05, UF-09 | — | trigger `trg_listing_facts_sync_cols` + hàm `listing_facts_sync_cols()` (AFTER INSERT `listing_facts` → `listings.bedrooms/area_m2/floor/direction`, chỉ ghi khi cột trống); migration `20260827e_listing_facts_sync_cols.sql`. Không đụng `description` | TS-SPECS-01…04 |
...
| FR-158 | UF-04, UF-05, UF-09 | — | bảng `bot_tokens` (RLS bật, không policy = chỉ service_role) + hàm `zalo_token_tick()` + cron `zalo-token-tick` `17 */12 * * *`; edge `zalo-token-refresh` (đổi `refresh_token`, giữ chuỗi cũ khi Zalo không trả chuỗi mới, ghi `last_error`); helper `zaloToken()` trong `_shared/claude.ts` thay mọi chỗ đọc `ZALO_OA_ACCESS_TOKEN` (`zalo-webhook`, `nudge`, `ask-seller`); `bot_tokens` vào danh sách `scripts/sao-luu.mjs`; migration `20260827j_token_zalo_tu_lam_moi.sql` | — |
| FR-159 | UF-05, UF-09 | — | helper `luuAnhVaoKho()` trong `_shared/claude.ts` (tải ảnh CDN Zalo → bucket `listing-photos/<mã>/chat-…`, trả URL công khai, hỏng thì `null` + ghi sổ); `chat-reply` dùng `anhUrl` thay `imageUrl` ở `messages.body`, `listing_facts.answer` và khối ảnh gửi model | — |
| FR-160 | UF-01, UF-02, UF-03, UF-04 | WF-01 | `zalo-webhook` xử `follow` (đọc `ref` từ `follower.ref`/`info.ref`/`ref`) và `unfollow` thay vì `return` sớm; `chat-reply` nhận `system_event` + `ref`, dựng câu mở đầu từ phía khách cho `follow`, ghi `messages` sender `system` + huỷ `reminders` pending cho `unfollow`; nhãn "(ghi chú hệ thống)" trong ngữ cảnh model | — |
| FR-161 | UF-04, UF-05 | — | hàm `wardNumberOf()` + hằng `WARD_RE` trong `chat-reply`, dùng chung cho nhánh câu rao người bán (`listings.ward`) và bộ lọc kho người mua (`ilike ward`) | — |
| FR-162 | UF-04 | — | bảng `rate_counters` + RPC `bump_rate(key, limit, window_secs)` (cửa sổ cố định, tự dọn > 3 ngày, khoá rỗng thì không chặn); CỔNG 3 trong `chat-reply` chỉ áp cho lượt không mang service_role; secret `DAILY_IP_MSG_CAP` (mặc định 300/24h); migration `20260827k_tran_theo_nguon_goi.sql` | — |
| FR-163 | UF-06, UF-08, UF-09 | — | `nudge` §1: chặn trước khi gọi model khi thiếu Zalo ID (→ `cancelled`) hoặc thiếu token OA (→ giữ `pending`); chỉ ghi `messages` + đóng `sent` khi `sent` là `zalo_oa`/`test`. `nudge` §2 (`reengage`): cùng cổng, không ghi dòng `reengage` ảo | — |
| FR-164 | UF-09, UF-13 | WF-08, WF-10 | `app/quan-ly/page.tsx`: state `deal` + ô xổ xuống Bán / Cho thuê, insert dùng `deal` thay hằng `"ban"` | — |
| FR-165 | UF-02 | WF-03 | `FACT_LABEL` + `factLabel()` trong `lib/format.ts` (24 khoá khớp `required_facts.fact_key` + `hinh_anh`); `app/nha-dat/[code]/page.tsx` khối "Đã xác minh với chủ nhà" | — |
| FR-166 | UF-11 | — | bảng `DICH_HOP_LE` trong `app/dang-nhap/page.tsx` (danh sách trắng, chống open-redirect); `app/admin/page.tsx` trỏ `/dang-nhap?next=admin` | — |
| FR-167 | UF-11 | — | `app/admin/page.tsx`: bọc `try/catch` quanh `getUser()` + truy vấn `admins`, rơi về `anon` kèm thông báo lỗi thay vì kẹt `loading` | — |
| FR-168 | UF-12 | — (chưa có wireframe cho `/tai-khoan` và `/yeu-thich`) | `app/tai-khoan/page.tsx` xếp hạng máy-này-trước / thiết-bị-khác-sau thay `indexOf` (tránh `-1` nhảy lên đầu); `app/yeu-thich/page.tsx` sắp lại theo mảng localStorage, mới nhất lên đầu; `components/TrackView.tsx` khai rõ `onConflict` | — |
