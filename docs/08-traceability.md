...
|---|---|---|
| DH-01 | Định vị hợp nhất: thương hiệu Aioinhadat, một bot Thái; địa bàn Sài Gòn (phường mới) + Long An, trọng tâm bán; mặt bán theo SRD AOND, mặt mua + web theo nhadat.cc, vòng nối qua CTV | OPEN-08 / OPEN-39 (chốt 03/09/2026), OPEN-27 nửa đầu (03/09/2026) / **FR-174** / BR-01 / INS-08 / INS-12, `06 §6.8`, INS-01, INS-06, FR-140 c, **FR-173**, FR-157, FR-159, SRS-5.1 |
...
| FR-152 | UF-09 | — | bảng `bot_errors` + `bot_health` (RLS, admin đăng nhập đọc được); RPC `beat(who)` gọi từ `escalation-feed`; `bot_health_tick()` quét `net._http_response` → `bot_errors` → `reminders` kind `escalation` (gộp 1 tin/giờ); cron `bot-health-tick` `*/15 * * * *`; RPC `log_loi(source, detail, code)` (van 20/nguồn/giờ + 200/giờ) + helper `ghiLoi()` trong `_shared/claude.ts` và trong `bridge-zca`; `escalation-feed` action `log`; web `instrumentation.ts` `onRequestError` + `app/error.tsx`; khối "Sức khoẻ bot" trên `/admin`; (e) migration `20260904a` `canh_bao_ngoai()` + `app_config.ntfy_topic`, `bot/bridge-zca/VPS.md` + `nhadat-bridge.service` | TS-HEALTH-01…08 |
...
| FR-174 | UF-04, UF-09, UF-10 | — | `_shared/dia_ban.ts` (`bocQuan`) + `chat-reply` `district` khi tạo tin; migration `20260903d` (`admin_dang_tin` nhận `district`); `_shared/prompts.ts` TONE_RULES + `bot_prompts.tone_rules`; copy web `app/layout.tsx`, `app/page.tsx`, `components/Footer.tsx`, `components/ListingBrowse.tsx`, `/mua-ban`, `/cho-thue`, `/ban-do`, `/moi-gioi`, `/raoban`, form `/admin/dang-tin`, ô quận `/quan-ly` (04/09); e2e DIABAN-01…03. SRS-3.1 (`listings.district`), SRS-3.12 (hàm). INS-08, INS-12; OPEN-27; BR-01, BR-02, FR-118 | TS-DIABAN-01…04 |
