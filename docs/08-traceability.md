...
| FR-175 | — | — | `20260907c` schema `so` (`so.ro_hang`, `so.nguoi_ban`); `app/admin/ro-hang/page.tsx` (lọc, sắp, tìm không dấu, CSV BOM, không SĐT); link từ `/admin`. Đọc qua RLS `listings_admin_read` + `sellers_admin_read` | — |
| FR-97 | | — | — | — |
