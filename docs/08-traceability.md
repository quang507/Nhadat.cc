...
| FR-204 | UF-04 | — | `20260911g`: cột `listings.toa_do_muc/geocode_at/tien_ich_gan/tien_ich_at`, `projects.geocode_at`, bảng `tien_ich`, `khoang_cach_m()`, `kd_ten()`, `moc_khop()`, `co_moc()`, `tin_gan_moc()`, `tin_can_geocode()`, `du_an_can_geocode()`, `geocode_tick()` + cron `geocode-tick`, trigger `trg_listings_zz_toa_do`; `geocode-listings` (Nominatim + Overpass + dự án), `_shared/geocode.ts`, `_shared/extraction/tien-ich.ts`, `_shared/ai/boc-gan.ts`, `_shared/tim-moc.ts`; `chat-reply` lọc KHO + dòng khoảng cách + tiện ích quanh căn. Kiểm: TS-GAN-01…03 | — |
