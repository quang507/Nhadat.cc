...
| TS-DUONG-03 | FR-212 DB thật sau nạp: `select tinh, count(*), count(distinct ten) from duong group by 1`; `tim_duong('pham the hien','Quận 8')` · `tim_duong('pham the hier')` · `tim_duong('le van viet')` · `tim_duong('abc')`; khoá publishable `GET /rest/v1/duong` và `POST /rest/v1/rpc/tim_duong` | ba tỉnh đều có dòng; Phạm Thế Hiển kc 0 kèm Quận 8; kc 1; Lê Văn Việt kèm Quận 9; rỗng; anon bị chặn (401/permission denied) | ✅ 21/09 (lượt đầu): TP.HCM 3.629 dòng / 2.696 tên / 71 phường · Tây Ninh 1.599 · Đồng Nai 685; `pham the hien`+Quận 8 → Phạm Thế Hiển kc 0 [Quận 8]; `pham the hier` → kc 1, một ứng viên; `le van viet` → Lê Văn Việt kc 0 [Quận 9] (+ 4 tên kc 1–2 ở Tây Ninh/Đồng Nai — `chonDuong` bỏ vì đã có khớp đúng); `abc` → rỗng; 56 ms; khoá publishable: GET `/rest/v1/duong` → 401 `permission denied for table duong`, POST `/rest/v1/rpc/tim_duong` → 401 `permission denied for function tim_duong` |
