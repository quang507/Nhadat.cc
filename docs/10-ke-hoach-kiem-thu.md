...
**Mười sáu bộ CHẠY MÁY, offline (543 ca) — `bun run kiem` gọi hết, CI chạy hết:**
...
| `bot/tests/giong/kiem-giong.mjs --tu-kiem` | 17 | `bun run test:bot` | **TS-GIONG-00** | Luật máy chấm giọng (≤30 từ, không "anh/chị" gạch chéo, không câu sáo/markdown/tên trường/mã tin/SĐT, không hỏi dồn, xưng cháu với chú/cô/bác): câu đúng mẫu đạt hết, câu sai mẫu rớt ĐÚNG luật |
| `bot/tests/giong/cham.mjs --tu-kiem` | 4 | `bun run test:bot` | **TS-GIONG-00** | Khuôn prompt giám khảo + JSON bốn cờ (nghe như người · đúng ý · không bịa · gọn) |
...
| TS-NGUOI-B04 | A | Đổi ý giữa chừng: `à mà nhà này cho thuê chứ ko bán, 25 triệu` | 🤖 Đã lưu: … **cho thuê** · giá 25 triệu (một dòng đầy đủ, từ 21/09 không còn "📦 Tin giờ") | Tin vẫn "bán", giá không đổi, hoặc bot nói "đã ngưng rao" |
...
| TS-V48-114c | FR-114 (e): khách mua có hồ sơ Quận 5 hỏi "quận 5 có dự án gì không em" (kho mock: Lakai + Dragon Riverside Q5, Sunrise City Q7, Ny'ah Q8 đối tác); câu không nhắc dự án khi kho có tin (10b) | ngữ cảnh model có khối "DỰ ÁN TRONG QUẬN 5" với 2 dự án Q5, không có Q7; model kể "Sunrise Quận 5" (bịa) → gọt còn "dự án", tên thật giữ; 10b không nạp khối | ✅ 22/09 e2e GVA-10/10b. **Bắn thật 04:05 UTC sau deploy #184** (`thu-m114-b`, đúng câu chủ dự án gõ trên Zalo): "Dạ anh, Quận 5 bên em có vài dự án đang bán như Hùng Vương Plaza (126 Hùng Vương, P12), Stella Residence (927 Trần Hưng Đạo, P1), hay Chung cư 155 Nguyễn Chí Thanh (P9) ạ." — ba tên + địa chỉ đều đúng dòng `projects` Quận 5 (priority 50, `status_text` "đang bán"); `bot_errors` 0. **Lộ một chỗ**: model nói "bên em có vài dự án ĐANG BÁN" và QUÊN câu "chưa có căn nào đang rao" dù được dặn → vá cùng lượt: kho không có căn nào thuộc các dự án vừa nạp thì code nối bong bóng tiền định "Hiện bên em chưa có căn nào của các dự án này đang rao, có căn là em báo anh liền ạ." ngay sau bong bóng kể dự án (e2e GVA-10 cập nhật). **Bắn lại 04:11 UTC sau deploy #185** (`thu-m114-c`): "Dạ anh, Quận 5 bên em có vài dự án đang mở bán: Hùng Vương Plaza (126 Hùng Vương, P12), Stella Residence (927 Trần Hưng Đạo, P1), Chung cư 155 Nguyễn Chí Thanh (P9), Lakai (5 Nguyễn Tri Phương, P7) với Lottery Tower (77 Trần Nhân Tôn, P9) ạ" + "Hiện bên em chưa có căn nào của các dự án này đang rao, có căn là em báo anh liền ạ." + câu hỏi giá/loại ✅ (model kể 5 thay vì tối đa 3 như dặn — nhỏ, chưa siết). Dữ liệu thử đã xoá |
...
| TS-BLDL-01 | FR-207: e2e `bot/tests/e2e/run.mjs` BLDL-01…11d | 💾 là bong bóng ĐẦU; "Vừa lưu" chỉ fact của lượt (không kèm "6x11" lượt trước), nguyên văn đã lưu; 📦 Tin giờ đúng cột DB (6m² không phải 66) và chỉ kèm khi tin đổi; lượt tạo tin bỏ 📝 giữ "Sai chỗ nào"; tin không lưu gì → không 💾; người mua bật công tắc → "Đã lưu nhu cầu" đúng khoá đã vào DB, không khoá nội bộ; tắt → không 💾; model hai nhánh không thấy 💾. **Từ 21/09**: lượt sau in MỘT dòng "🤖 Đã lưu: <toàn bộ tin>" (6m² đúng cột, có địa chỉ/phường/giá/pháp lý/hướng) + "Kèm:", KHÔNG còn "📦 Tin giờ" (BLDL-03/03b/08/10/10b viết lại) | ✅ 14/09, e2e 318/318 · ✅ 21/09 e2e 439/439 |
...
| TS-AIBOC-08 | FR-208 (g) offline: `bun bot/tests/vi-du-boc-rao.mjs` — 8 ví dụ mẫu trong prompt, mọi trường qua `kiemDeXuat`, mọi kiến thức qua `kiemKienThuc`, khoá thuộc `MOI_KHOA`, bản chữ ≤ 8000 ký tự, không SĐT; `kiem-bang-chung.mjs` khớp mờ: 'đường Châu Văn Liên' (lệch 1) đạt + ghi `trich_dan_sua`; 'dài 16m' khi tin 'dài 18m' bỏ (chữ số khác); lệch 4 bỏ; cụm < 10 ký tự không khớp mờ; khớp mờ vẫn qua lớp 2 (tiền không khớp vẫn bỏ); thu nhập thuê toà nhà bán đạt, tin cho thuê bỏ | 0 ví dụ dạy thứ code bỏ; khớp mờ không nới chữ số | ✅ 21/09 vi-du 34/34, kiem-bang-chung 116/116 |
| TS-AIBOC-09 | FR-208 (g) bắn lại trên production 21/09/2026 07:09–07:12 UTC (deploy #164, `mau-u-01…04`): 4 câu rao cùng dạng 4 câu model từng đọc sai ở TS-AIBOC-07 (đất vườn Củ Chi 1800m2 / 200m2 thổ; biệt thự khu An Phú Thủ Đức 12x20; toà CHDV Bình Thạnh thu nhập 120 triệu/tháng; phòng trọ không dấu 22m2 2tr5/thang) + 1 câu nối "ngang 12 dài 20 nha, hẻm xe hơi, để em coi lại sổ rồi báo" (câu treo phường) | `dai` không còn = diện tích; không `du_an` từ tên khu; `thu_nhap_thue` đạt; `dien_tich` số trần; không hiện trạng "xe hơi", không kiến thức lời hứa; sổ đo `bo` giảm | ✅ 5/5 HTTP 200, 0 `bot_errors`. Sổ đo 4 câu rao: **30/30 đề xuất đạt, 0 bỏ** (lượt TS-AIBOC-07 cùng dạng: 5 bỏ). Đất vườn: `dien_tich = 1800`, không `dai`, "có 200m2 thổ" → kiến thức; biệt thự: không `du_an`, ngang 12 / dài 20 / 4 PN / 75 tỷ / full / sổ hồng riêng; toà CHDV: `thu_nhap_thue = 120 triệu` ĐẠT, `ket_cau = hầm 5 tầng`, hẻm 6m, "18 phòng full khách" → kiến thức; phòng trọ: `dien_tich = 22` (số trần), `gia = 2tr5`, cọc 1 tháng, 3 kiến thức không dấu. Câu nối: chỉ `dien_tich = 12x20` nguồn ai_kiem, câu phường vẫn treo. **Vết còn**: `thu_nhap_thue` đạt kiểm nhưng không có chỗ ghi (`KHOA_GHI` thiếu) → `rent_income_vnd` trống, luật ghi `doanh_thu` thay — vá cùng lượt: `thu_nhap_thue` → fact `doanh_thu`. Fact `san_vuon` luật vẫn ghi cả câu (chưa vá, ngoài phạm vi). Dữ liệu `mau-u-*` đã xoá. |
| TS-AIBOC-10 | FR-208 (h): offline `kiem-bang-chung.mjs` (phục hồi dấu 'Phạm Thế Hiển' ↔ 'pham the hien' đạt; đổi chữ cái 'Hiếu' bỏ; câu vị trí lấy `duong`, không có thì null) + `vi-du-boc-rao.mjs` ví dụ 9; e2e AIBOC-14 (chế độ `chinh`, câu treo vị trí, khách gõ cả câu không dấu "nhà của anh ở hem 4m Pham The Hien, P.4 📐") và AIBOC-15 (AI rỗng → luật đỡ '123/4 An Dương Vương'); AIBOC-16 (câu treo loại BĐS: RPC đoán loại trả null, AI đọc 'dat' → ô loại 'đất', không hỏi lại); bắn lại thật cùng câu sau deploy | vi_tri = 'Phạm Thế Hiển' có dấu, hẻm 4m và phường 4 đi ô riêng, không fact nào mang cả câu; AI rỗng thì luật vẫn ghi | ✅ 21/09 offline 122/122 + 38/38, e2e 462 ✓; bắn thật 07:40 UTC sau deploy #166 (`mau-t-01`: rao "anh bán nhà quận 8, 4x12, 1 trệt 2 lầu, giá 5 tỷ 2" → bot hỏi phường; trả lời "nhà của anh ở hem 4m Pham The Hien, P.4 📐") → `street` = `location_raw` = "Phạm Thế Hiển" (có dấu), `ward` = Phường 4, `alley_width_m` = 4, fact `vi_tri` + `do_rong_hem` nguồn ai_kiem, câu phường answered, câu kế "bao nhiêu phòng ngủ"; 0 `bot_errors`. Sổ đo AI: duong «Pham The Hien» → "Phạm Thế Hiển", do_rong_hem «hem 4m», phuong «P.4». Tin thật của chủ dự án lúc test (BDS-NP-Q8-0001, trước deploy) vẫn mang street cả câu — sửa tay hoặc chủ nhắn lại địa chỉ. Dữ liệu `mau-t-*` đã xoá. |
...
| TS-DUONG-01 | FR-212: `bun bot/tests/tra-duong.mjs` — chọn kết quả tra: khớp đúng → sửa (kể cả IN HOA); đã đúng → giữ; hai tên cùng chữ bỏ dấu → giữ, biết quận → chọn tên trong quận; khớp gần 1 ứng viên → hỏi; hai ứng viên cùng khoảng cách → giữ (biết quận → hỏi tên trong quận); tên ngắn / lệch 2 mà < 9 chữ cái / khoảng cách 3 → giữ; ứng viên rác → giữ; thay tên trong địa chỉ giữ số hẻm/phường; câu xác nhận < 30 từ | 25/25 | ✅ 21/09 |
| TS-DUONG-02 | FR-212: e2e DUONG-01…07 (bảng `duong` seed 3 dòng, RPC giả chép ngữ nghĩa `tim_duong`): không dấu → có dấu ngay, tra đúng 1 lần bằng tên trần + quận; sai 1 ký tự → câu đầu là xác nhận, gợi ý cất, địa chỉ chưa sửa; gật → sửa + "Dạ em sửa lại …" + hỏi lại câu treo; không gật, nói giá → gợi ý bỏ, giá vẫn ghi; từ điển trống → giữ, 0 `bot_errors`; trả lời địa chỉ không dấu khi bot hỏi phường và khi câu vi_tri treo → sửa dấu; RPC hỏng → 1 dòng sổ lỗi, địa chỉ vẫn ghi | 8 ca | ✅ 21/09 (422/422 e2e) |
| TS-DUONG-03 | FR-212 DB thật sau nạp: `select tinh, count(*), count(distinct ten) from duong group by 1`; `tim_duong('pham the hien','Quận 8')` · `tim_duong('pham the hier')` · `tim_duong('le van viet')` · `tim_duong('abc')`; khoá publishable `GET /rest/v1/duong` và `POST /rest/v1/rpc/tim_duong` | ba tỉnh đều có dòng; Phạm Thế Hiển kc 0 kèm Quận 8; kc 1; Lê Văn Việt kèm Quận 9; rỗng; anon bị chặn (401/permission denied) | ✅ 21/09 (lượt đầu, chốt 10:35 UTC): TP.HCM 4.830 dòng / 3.364 tên / 94 phường · Tây Ninh 1.599 · Đồng Nai 685 = 7.114 dòng, 5.162 tên; `pham the hien`+Quận 8 → Phạm Thế Hiển kc 0 [Quận 8]; `pham the hier` → kc 1, một ứng viên; `le van viet` → Lê Văn Việt kc 0 [Quận 9] (+ 4 tên kc 1–2 ở Tây Ninh/Đồng Nai — `chonDuong` bỏ vì đã có khớp đúng); `abc` → rỗng; 56 ms; khoá publishable: GET `/rest/v1/duong` → 401 `permission denied for table duong`, POST `/rest/v1/rpc/tim_duong` → 401 `permission denied for function tim_duong` |
| TS-DUONG-05 | FR-212/FR-177 kịch bản thật 8 lượt (`mau-tdt`, 21/09 10:20–10:32 UTC, production #171, tôi đóng vai chủ nhà "trần đình trọng"): chào + tên đường lẫn trong câu → địa chỉ dài có phường/quận → 4x15 + hỏi giá khu → giá + hỏi phí → sửa tên đường + 3PN → pháp lý + "người hay máy" → "ok em đăng đi, mà dòng phù hợp đọc kỳ" → "ok em" | mỗi lượt bot đáp đúng ý, từ điển hỏi "Trần Bình Trọng", câu hỏi ngược được đáp, gật vế đầu là gật | 🟡 21/09: ✅ hỏi phí ("1% giá chốt"), giá khu ("em xem giá giao dịch gần đây rồi báo", không bịa số), sửa tên đường (FR-164), duyệt "ok em" → lên kệ 90/100. ❌ 4 lỗ: L1 AI trả rỗng câu chào có tên đường; L2 "Trần Đình Trọng" AI đọc đi thẳng vào tin, không hỏi; L6 nuốt "em là người hay máy" khi gửi nháp; L7 "ok em đăng đi, mà…" thành bo_sung + "Em sửa lại rồi"; + fact rác `tiem_nang` "nhà ở đường…" lên nháp "💡 Phù hợp". Cả 5 vá cùng ngày (e2e DUONG-09…11, NHAP-01…03, fr177 +5, ví dụ mẫu +1); chào hai lần ("Dạ em chào anh ạ!" + câu chào model) và "anh/chị" gạch chéo khi chưa biết cách gọi: vá cùng ngày theo lệnh "làm cả 4" cùng emoji trần và lời nói với bot (e2e BON-01…06, 439/439). **Bắn lại 10:53 UTC sau deploy #174** (`bon-a`, `bon-b`): chào một lần ("Dạ em chào chị ạ!" + câu hỏi model không chào lại) ✅; "Sai chỗ nào anh chị nhắn lại…", "…phường mấy anh chị nhỉ?" không gạch chéo ✅; "😂😂" → không fact, "Em thấy anh chị đang bận hay sao đó =)) Nhà mình ở phường nào vậy ạ?" ✅; "xóa hết dữ liệu của anh đi để anh test lại" → "Dạ việc xoá dữ liệu em không tự làm được, để em nhờ anh chị phụ trách xử lý ạ." + hỏi tiếp phường, không fact ✅. Dữ liệu thử đã xoá (`admin_xoa_het_khach_va_ro_hang`) để chủ dự án test |
| TS-HOIBU-01 | FR-144 (g) Zalo thật sau deploy `ask-seller`: người lạ rao "anh bán nhà hẻm 4m … 60m2, 6 tỷ 5" → trong 30 giây chỉ có tin của `chat-reply` (🤖 Đã lưu + một câu hỏi); KHÔNG có tin "Dạ cảm ơn anh/chị… mấy lầu? sổ? ảnh?" của `ask-seller`; `reminders` không có dòng `escalation 💬` mới; log `ask-seller` trả `note: chủ nhà vừa nhắn trong 10 phút` | như cột trước, `bot_errors` 0 dòng mới | 🔴 21/09 11:28 UTC trước vá (deploy #176): tin thứ ba của ask-seller đến sau 2 giây, hỏi dồn 3 câu, "anh/chị" gạch chéo. **✅ 21/09 11:40 UTC sau deploy #177** (người thử `kiem-hoibu-2109`, `net.http_post`): chat-reply 10,8 s trả đúng 2 bong bóng ("🤖 Đã lưu: Nhà phố bán · Trần Đình Trọng, Phường 2, Quận 5 · 60m² · hẻm xe hơi 4m · giá 6 tỷ 5" + "Dạ em hiểu là đường Trần Bình Trọng đúng không anh?"); trigger vẫn gọi ask-seller (3,4 s, 200) nhưng KHÔNG gửi gì — lượt này ask-seller khởi động lạnh 2,5 s sau nên rơi vào cổng cũ `skipped_pending: ["ket_cau"]` (câu chờ đã mở); cổng "chủ nhà vừa nhắn 10 phút" chưa được kích trên production, chỉ qua deno check — `so.hoi_thoai` 3 dòng, `reminders` 0, `info_requests` chỉ `ket_cau` của chat-reply, `bot_errors` 0; dữ liệu thử đã xoá bằng `reset_nguoi_test` |
| TS-DUONG-06 | FR-212 Zalo thật: rao ĐÚNG tên "anh bán nhà hẻm 5m Trần Bình Trọng phường 1 quận 5, 4x15, 7 tỷ 2" | 🤖 in "Trần Bình Trọng" nguyên văn, KHÔNG hỏi "em hiểu là…", `duong_goi_y` null, hỏi tiếp câu treo kế; lượt 2 "1 trệt 2 lầu, 3 phòng ngủ" → một dòng đầy đủ có "trệt + 2 lầu · 3 phòng ngủ", không 📦 | ✅ 21/09 13:48 UTC (deploy #176, `kiem-tbt`): "🤖 Đã lưu: Nhà phố bán · Trần Bình Trọng, Phường 1, Quận 5 · 60m² · 4x15m · hẻm xe hơi 5m · giá 7 tỷ 2" + "Nhà mình xây mấy tầng rồi anh?"; lượt 2 đúng, `floors=3`, `bedrooms=3`, điểm tin 75. Lòi: "Kèm: số phòng ngủ: 3" lặp dòng chính (fact AI có cột) → vá `kemLuotTao` chỉ nêu fact ngoài cột |
| TS-BLDL-04 | FR-207 + FR-176 Zalo thật: chú lớn tuổi rao căn hộ "chào cháu, chú có căn hộ Sunrise City tầng 15 view sông muốn bán, 2 phòng ngủ 76m2, 5 tỷ 8"; lượt 2 "để lại full nội thất cháu, chú bán vì cần tiền cho con du học" | xưng "cháu"; dự án gắn `Sunrise City` → Phường Tân Hưng, Quận 7 tự điền; `floor=15`; 🤖 là bong bóng ĐẦU, KHÔNG có "📝 … ghi nhận" trùng; "Kèm: view…"; lượt 2 một dòng đầy đủ có "nội thất đầy đủ", Kèm có "cần tiền cho con du học" | 🟡 21/09 13:48 UTC (deploy #176, `kiem-cc`): mọi ý ✅ trừ một: lượt tạo in cả "Dạ cháu chào chú ạ!\n📝 Cháu ghi nhận: …" lẫn 🤖 — `doiTuXung` đổi "Em"→"Cháu" TRƯỚC đoạn bỏ 📝 (chỉ nhận "📝 Em ghi nhận"). Vá: so theo dấu 📝 + "ghi nhận" (e2e BLDL-06c, 440/440). Lượt 2 ✅: "🤖 Đã lưu: Căn hộ bán · Phường Tân Hưng, Quận 7 · dự án Sunrise City · 76m² · tầng 15 · nội thất đầy đủ · 2 phòng ngủ · giá 5 tỷ 8 / Kèm: thông tin bổ sung: "cần tiền cho con du học" · nhãn tìm kiếm: "full nội thất"". **Bắn lại 21/09 13:57 UTC sau deploy #178** (`kiem-cc2`, đi tới duyệt): lượt 1 hết 📝 trùng ✅ (3 bong bóng: 🤖, "Dạ cháu chào chú ạ!", câu hỏi); lượt 3 "sổ hồng riêng rồi cháu, chú đứng tên" → điểm 78 ≥ 70 → bot gửi bản nháp 📋 "Cháu đăng tin như vầy nha chú…" ✅; lượt 4 **"ok đăng đi cháu" KHÔNG gật** ❌ — vào "📝 Thêm: ok đăng đi cháu", nháp gửi lại, `chu_duyet_at` null. Nguyên nhân: bộ từ đệm `laDongY` có anh/chị/em/mình mà thiếu cháu/chú/cô/bác/con. Vá + 5 ca phải-kích, 4 ca không-được-kích (`khong-duoc-kich.json`), e2e NHAP-04 (441/441). **✅ Bắn lại 21/09 14:09 UTC sau deploy #179** (`kiem-cc3`, rao đủ một câu 78/100): lượt 2 trả lời hướng → bot hỏi tiếp phí quản lý (chưa gửi nháp vì còn câu chuyên môn); lượt 3 "ok đăng đi cháu" lúc đang hỏi phí → hiểu là chủ muốn đăng, đóng câu phí, gửi 📋 ngay; lượt 4 "ok đăng đi cháu" → "Dạ cháu cảm ơn chú! Chúc mừng chú, tin nhà mình đã được ghi nhận…", `chu_duyet_at` đặt, `status = dang_ban`, `bo_sung` 0 dòng "đăng đi", `bot_errors` 0 dòng của bot. Deploy #179 lần 1 chết ở bước cài Supabase CLI (GitHub rate limit khi tra bản mới nhất), chạy lại lần 2 xanh |
| TS-BLDL-05 | FR-176 (c) lời chào (22/09, chủ dự án: "người ta chào là cô chào cháu nó vẫn đáp anh chị"): tin đầu "cô chào cháu" → lời chào gọi cô, xưng cháu; "chào cháu" trơ → xưng cháu, gọi "mình", hỏi "cháu gọi chú hay cô"; trả lời "cô" trơ → ghi, hỏi lại vai; câu rao sau đó (có hay không xưng) → hồ sơ bán mang "cô"/lớn tuổi sang; chủ nhà có tin nhắn "cô chào cháu" → nhận "cô", không ghi fact; khách mua là chú → nhánh mua xưng cháu. Máy: e2e GVD-01…08; `boc-tach-42-ca.mjs` +24 (tuXungTuCau lời chào, batXungHo trơ có dấu, laChaoChau, CHAO_SUONG_RE) | mỗi ca đúng như tên; không câu nào còn "anh chị"/"em" với người lớn tuổi | ✅ 22/09 (máy). Trước vá: "cô chào cháu" tin đầu → "Dạ em chào anh chị, em là …" (luật chỉ bắt chú/cô/bác + động từ có/cần/bán, "chào" không có trong bộ). Bắn thật: xem cột này cập nhật |
| TS-GIONG-01 | FR-180 (f) / OPEN-54: `bun run giong:gia` — 24 ca chạy handler thật với model GIẢ: câu ĐÚNG mẫu (oracle) và câu SAI mẫu (null) | oracle ≥ 20/24; null ≤ 8/24; phần rớt của oracle chỉ ra câu TIỀN ĐỊNH lệch luật giọng | ✅ 22/09 (sandbox): oracle **21/24**, null **5/24**. Ba câu tiền định rớt luật ≤30 từ: lời chúc mừng duyệt tin (92 từ, có gạch dài), "bán rồi" (35 từ), "không gửi số chủ nhà" (37 từ) — chờ chủ dự án chốt có rút gọn không. Lòi lỗi thật: nhánh MUA chưa qua bộ lọc gạch chéo (câu dò "Anh/chị cho em xin thêm…") → vá cùng lượt, e2e BON-07 |
| TS-GIONG-02 | FR-180 (f): `bun run giong` (Haiku 4.5, model production) rồi `bun run giong:cham` (giám khảo Opus 5) trên máy có `ANTHROPIC_API_KEY` | TẦNG 1 ≥ 20/24 lượt đạt mọi luật máy; TẦNG 2 mỗi cờ ≥ 20/24; `errors.jsonl` 0 dòng; chạy `--lap 2` để thấy độ lệch giữa hai lần | ✅ **22/09 01:34 UTC chạy thật** (khoá chủ dự án đưa trong chat, chỉ đặt vào env của tiến trình; deploy #180): TẦNG 1 **18/24** (rớt 5 ca luật ≤30 từ: B08 40 từ, B09 92, B13 35, B14 33, M03 37; B16 rớt luật riêng quá chặt → nới "giá|bao nhiêu|thu về"), 0 lỗi API, token vào 14,5k + cache đọc 176k, ra 4,3k. TẦNG 2 lần 1 (giám khảo không thấy trạng thái dựng sẵn) "không bịa" 11/23 — hơn nửa oan (địa chỉ tin, luật phí, kho hàng bot đọc từ DB bị coi là bịa) + M03 cắt JSON → giám khảo nay được đưa ngữ cảnh hệ thống + bảng 🤖, `max_tokens` 800. TẦNG 2 lần 2: **nghe như người 20/24 · đúng ý 17/24 · không bịa 17/24 · gọn 21/24**, 0 lỗi, giám khảo Opus 5 vào 13,2k ra 4,3k. **Mười lỗi thật lòi ra** (sổ việc, chưa vá): (1) B08 bong bóng "em là trợ lý AI" in HAI lần — câu tiền định + model lặp lại nguyên văn rồi hỏi dồn sổ + hoàn công; (2) B11 môi giới đã nói "hẻm 4m" và "mặt tiền" cho hai căn mà bot hỏi lại hẻm; (3) B13 câu "bán rồi" tiền định nói "đã báo các khách đang chờ" khi chưa có khách nào — template bịa; (4) B14 câu lủng củng "gửi ảnh lên để tiếp khách hỏi được thì tốt hơn"; (5) M02 kho có căn 12 Trần Hưng Đạo hẻm 6m gần chợ Hoà Bình khớp mà bot nói "chưa có căn nào" (chưa rõ do mock kho hay do bot — cần bắn thật); (6) M03 bong bóng 2 in chữ "[liên hệ qua Zalo]" — bộ lọc liên hệ FR-105 che mất chữ của chính bot; (7) M04 khách mua có hồ sơ hỏi "còn bán không" mà bot đáp như với NGƯỜI BÁN ("nhà mình là nhà phố hay chung cư?") — lạc vai; (8) M06 căn hẻm 6m mà bot nói "mặt tiền kinh doanh" + khẳng định mở quán được; (9) B01/B15/B16 bot tự thêm "ô tô vào được", "xuyên thoáng", "khách chuộng" khi chủ chưa nói (TONE_RULES cấm khen điều khách không nói); (10) B09 câu chúc mừng duyệt tin 92 từ, giọng "đã được ghi nhận", B04 sửa giá chỉ hiện ở bảng 🤖 không có lời xác nhận — hai điểm chờ chủ dự án chốt là đúng thiết kế hay sửa. Số này là **đường nền** để so mọi lần sửa prompt/luật sau |
| TS-GIONG-03 | FR-180 (f): vá mười lỗi của TS-GIONG-02 rồi đo lại. Máy: e2e GVA-01…07 (M04 khách mua "còn bán không" không mở hồ sơ bán; M03 chữ "qua Zalo" của bot không bị che, ID/SĐT vẫn che; B08 "trợ lý AI" một lần; B13 "báo các khách đang chờ" chỉ khi `interests` có; B11 giá "5 tỷ" không nuốt "60m2", hẻm/mặt tiền ghi riêng từng căn; B04 giữ "Dạ em sửa lại giá … rồi ạ" cạnh 🤖; M06 câu "mặt tiền" trái kho hẻm 6m bị bỏ) + H8e (câu chúc hai câu ≤ 30 từ, cách thêm điểm tách bong bóng) + `van-tra-loi.mjs` 12 ca ba lưới mới (`boCauTrung`, `boKhenKhongCanCu`, `boMauThuanCan`) + `luat-lien-he.mjs` 4 ca `thayLienHeCoId` + `boc-tach-42-ca.mjs` 3 ca. Người: `bun run giong` + `giong:cham` có khoá, so với đường nền | e2e xanh; `giong:gia` (model giả tốt) ≥ 22/24; TẦNG 2 trên model thật: mỗi cờ ≥ 20/24 (đường nền 20 · 17 · 17 · 21) | ✅ **22/09 (máy)**: e2e **450/450**, van-tra-loi 187/187, luat-lien-he 35/35, boc-tach 273/273, luat-pha-du-lieu 118/118, `kieu:bot` sạch; `giong:gia` **22/24** (2 ca rớt là ca `--gia` không có `phai`), `giong:gia=xau` 6/24 (model giả xấu, đúng ý là lưới không bắt hết — xem cột luật). Vá ở ĐƯỜNG RA, không đổi prompt trừ hai chỗ: `dang_xong` rút còn hai câu (bỏ câu hẹn `dang_xong_hen` khỏi lắp ráp) và thêm một ví dụ "có khách chưa" vào `SELLER_FEWSHOT` (đẩy DB `bot_prompts` cùng lượt). Luật khen không căn cứ chỉ áp cho lời MODEL (bản nháp / bảng đọc từ DB không qua) — lượt đầu áp cả bong bóng tiền định làm 📋 mất câu (H5/H6/N26 đỏ), là vết đáng nhớ. Mock DB nay gọt `price_raw` như `chuan_hoa_gia_raw` để bộ đo không thấy "7 tỷ 5 nha em" mà production không in. **Bắn thật 02:21–02:30 UTC sau deploy #181** (`thu-m02-s` rao "hẻm 6m 12 Trần Hưng Đạo p4 q5, 4x12.5, 3 lầu 3PN, SHR, gần chợ Hoà Bình, 5 tỷ 8" → không gấp → "ok em đăng đi"; `thu-m02-b` "tìm nhà quận 5 tầm 6 tỷ để ở" → "có căn nào hẻm xe hơi gần chợ không"; xoá bằng `reset_nguoi_test` sau khi đo, `bot_errors` 0 dòng mới): B09 ✅ hai bong bóng "Dạ em cảm ơn anh, tin nhà mình lên kệ … 85/100. Có khách quan tâm là em báo anh liền." + "Muốn thêm điểm…"; **(5) M02 ✅ kho TÌM THẤY** căn ("Dạ em có căn trên Trần Hưng Đạo P4, hẻm xe tải 6m … 50m2, 3PN, 5 tỷ 8, sổ hồng riêng") — lỗi ở bộ đo là do mock (`tin_gan_moc` chưa giả lập), không phải bot. **Hai lỗi mới lòi ra, vá cùng ngày (chủ dự án "ok vá đi, làm cả hai kèm e2e")**: (11) bot nói "gần chợ **Hàng Thịt**" trong khi fact `tien_ich_gan` = "gần chợ Hoà Bình" — nguyên nhân: dòng kho gửi model KHÔNG mang tiện ích lẫn nhãn, khách hỏi "gần chợ" nên model điền tên vào chỗ trống. Vá hai tầng: dòng kho thêm `boc_tach.tien_ich_gan` + nhãn tìm kiếm; lưới `boTenRiengBia` gọt tên riêng sau chợ/trường/bệnh viện/công viên/siêu thị/chùa/nhà thờ/bến xe không có trong ngữ cảnh (e2e GVA-09, van-tra-loi 4 ca). (12) `vi_tri` = "Trần Hưng Đạo", số nhà **12** rơi — nguyên nhân KHÔNG phải AI đọc sai: prompt dặn AI trả `duong` là tên đường trần (hẻm/phường đi ô riêng), còn luật `bocViTriRao` giữ "hẻm 6m 12 Trần Hưng Đạo"; chế độ `chinh` lấy AI trước nên số nhà mất. Vá `chonViTri`: số nhà đứng ngay trước tên đường trong bản luật + tên đường AI đã phục hồi dấu → "12 Trần Hưng Đạo"; "hẻm 4m" không phải số nhà; không số nhà thì AI thắng như cũ (AIBOC-14 giữ nguyên). Áp cả lúc tạo tin lẫn câu treo vị trí (e2e GVA-08, kiem-bang-chung 5 ca). **Bắn lại 03:18–03:22 UTC sau deploy #182** (cùng hai người thử, xoá bằng `reset_nguoi_test` sau khi đo; dữ liệu thật của chủ dự án đang thử song song — 1 tin · 1 người bán · 2 khách — không đụng): (12) ✅ `vi_tri` = "12 Trần Hưng Đạo", 🤖 và bản nháp đều có số nhà; (11) ✅ "Dạ em có căn 12 Trần Hưng Đạo P4 - hẻm xe tải 6m, **gần chợ Hoà Bình**, 5 tỷ 8, 50m2, 3PN ạ." — đúng chợ chủ nhà nói. `bot_errors` 1 dòng `geocode-listings overpass` (ba máy chủ Overpass đều hỏng lúc đó, chuyện của dịch vụ ngoài, không liên quan). **Lòi thêm hai lỗi nhỏ, chưa vá**: (13) cột `street` = "12 Trần Hưng Đạo" — `boc_ten_duong()` (DB) chỉ bỏ số nhà khi đứng sau "hẻm/đường", số nhà TRẦN đầu chuỗi giữ nguyên (`boc_ten_duong('12 Trần Hưng Đạo')` → "12 Trần Hưng Đạo", còn "hẻm 6m 12 Trần Hưng Đạo" → "Trần Hưng Đạo"); có sẵn từ trước với "7 Hồng Bàng", nay lộ vì địa chỉ giữ số nhà — sửa cần migration + sinh lại `schema.sql` (khoá service_role); (14) nhánh mua lượt đầu: 🤖 "Đã lưu nhu cầu…" rồi model lặp "Dạ em đã lưu nhu cầu: mua nhà Quận 5, tầm 6 tỷ để ở ạ" — `laCauGhiNhanMot` chưa nhận chữ "lưu". **Vá cùng ngày (chủ dự án "ok vá luôn 13 với 14")**: (13) migration `20260922b_boc_ten_duong_so_nha_tran` thêm một lớp bỏ số nhà trần đầu chuỗi (giữ "3 Tháng 2", "30 Tháng 4") — 14 câu thử trên DB thật đúng hết ("12 Trần Hưng Đạo"/"số 7 Hồng Bàng"/"123/4 An Dương Vương"/"12a Nguyễn Trãi P3" → tên đường trần; các câu cũ giữ nguyên), `soat-migration` md5 khớp; (14) thêm "lưu" vào động từ ghi nhận, "lưu ý" không tính (van-tra-loi 3 ca, e2e BLDL-11c). Đo lại TẦNG 2 trên model thật cần khoá API (khoá chủ dự án dán 22/09 phải thu hồi, không dùng lại) |
| TS-GIONG-04 | Kịch bản thật B (chủ dự án tự nhắn Zalo 22/09 11:27–11:34): môi giới rao căn hộ Hùng Vương Plaza gõ không dấu → hỏi phí kèm hướng → sửa giá → hỏi lại giá → pháp lý/nội thất/thang máy/view → "đủ rồi, đăng đi" → "có khách nào hỏi chưa" → "bán rồi". Máy: e2e GVA-11 (hỏi giá → đáp từ DB + học "anh"), GVA-12/12b (đang treo chấm điểm, hỏi khách → không nuốt; 0 và 1 khách), GVA-13 (`chinh`: "sổ hồng riêng" ở kiến thức thêm → luật ghi phap_ly, không hỏi lại), GVA-14 (số nhà "126" trước "p12" + 🤖 "4 tỷ 3"), N3b (bán rồi → xin chấm điểm), H10b ("đủ rồi" không xin điểm); `hoi-ve-tin.mjs` 27 ca; boc-tach 275 | mỗi ca đúng như tên | ✅ 22/09 (máy): e2e **461/461**; kieu/truyvet sạch. **Đạt trong kịch bản thật**: gắn dự án, phí 0,5% + hướng cùng lượt, sửa giá có lời xác nhận, nhãn, "bán rồi" không bịa "báo khách". **Sáu lỗi thật, vá cùng ngày**: (1) "có khách nào hỏi chưa em" bị nuốt làm điểm chấm; (2) "hồi nãy anh nói giá bao nhiêu nhỉ" → "em kiểm tra rồi báo lại" dù giá trong DB, "anh nói" không học cách gọi; (3) số nhà "126" rơi — hai nguyên nhân: luật `bocViTriRao` trượt phường hai chữ số ("p12"), và `chonViTri` bắt lần xuất hiện đầu của tên đường ("Hung Vuong **Plaza**"); (4) vừa lưu "sổ hồng riêng" bot hỏi lại pháp lý — DB thật cho thấy AI xếp câu đó vào kiến thức thêm (`bo_sung`, nguồn ai_kiem, ghi SAU khi câu kế đã mở), không trả khoá `phap_ly`; (5) câu chấm điểm ngay sau "đủ rồi, đăng đi" đứng sai chỗ (thiết kế 09/09) → dời sang lúc báo bán được; (6) 🤖 in "giá 4 ty 3" nguyên chữ. **Deploy #186 đỏ 401** — token `SUPABASE_ACCESS_TOKEN` trong GitHub Secrets bị thu hồi (chủ dự án thu hồi nhầm khi được nhắc thu hồi khoá Anthropic), chủ dự án cấp token mới → deploy #187 xanh. **Bắn thật 07:17–07:22 UTC** (`thu-b2`, đúng câu kịch bản B, xoá sau khi đo): (3) ✅ 🤖 "126 Hùng Vương, Phường 12, Quận 5"; (6) ✅ 🤖 "giá 4 tỷ 3"; (2) ✅ "Dạ giá mình đang rao là 4 ty 3 ạ." + hỏi lại nội thất, `xung_ho` = anh (còn "4 ty 3" thô trong câu trả lời → vá cùng lượt, `donViGiaDep` về `luat-tien.ts` dùng chung); (4) ✅ fact `phap_ly` = sổ hồng riêng (seller_chat), câu kế là HƯỚNG, không hỏi lại pháp lý; (1) ✅ "Dạ hiện chưa có khách nào hỏi căn này, có khách là em báo anh liền ạ." + hỏi lại hướng; (5) ✅ "bán rồi" → chúc mừng + gỡ tin + bong bóng xin chấm điểm, `danh_gia` pending. `bot_errors` chỉ 1 dòng `geocode-listings overpass` (dịch vụ ngoài) |
| TS-GIONG-05 | Kịch bản thật C (22/09, bắn thẳng production uid thử `thu-tbt-loi`, xoá sau khi đo): nhà hẻm Trần Bình Trọng gõ sai chính tả ("tran dinh trong") → sửa phường → sửa hẻm 4m→3m5 → sửa diện tích → "3pn 2wc, hoan cong 2019, gia 7 ti 5 chu ko phai 7 ty 2" → "😂😂" → "em xoá cái hẻm 4m ghi nhầm đi" → "bán rồi hả em?" → "đăng đi". Máy: e2e GVC-01 ("7 ti 5" đổi giá thật, xác nhận "7 tỷ 5"), GVC-02/02b (xin bỏ ô: không ghi, đọc lại ô, câu treo giữ), GVC-03 (so_wc → bathrooms), GVC-04 (một lời sửa), GVC-05 ("bán rồi hả em?" → "chưa bán", tin giữ), GVC-06 (🤖 "7 tỷ 2"), GVC-07 (emoji lúc duyệt); `van-tra-loi.mjs` +22, `hoi-ve-tin.mjs` +8, `tien.json` +4 | mỗi ca đúng như tên | **Đạt trong kịch bản thật (5/9 lượt)**: hỏi xác nhận tên đường gõ sai, sửa phường/hẻm/diện tích vào đúng cột, "bán rồi hả em?" không đóng tin. **Bảy lỗi thật, vá cùng ngày** (`20260922c` + chat-reply): (1) sửa giá "7 ti 5" không hiệu lực mà bot nói "đã sửa" — `parse_vnd`/`docTien` cùng thiếu "ti" nên cổng đối chiếu không đỏ; (2) "em xoá cái hẻm 4m ghi nhầm đi" thành địa chỉ, tin ĐĂNG LÊN WEB với street là nguyên câu, hẻm 3.5 bị đè lại 4; (3) `so_wc` không vào `bathrooms`; (4) hai lời "sửa lại Phường 2"; (5) "bán rồi hả em?" → model "để em hỏi chủ nhà xác nhận" với chính chủ nhà; (6) 🤖/📋 in "7ty2"; (7) "😂😂" → model "Em thấy anh chị đồng ý rồi". `bot_errors` 0 trong khung giờ. **Bắn lại 08:35–08:45 UTC sau deploy #189** (`thu-tbt-c2`, đúng 9 câu, xoá sau khi đo): (6) ✅ 🤖 "giá 7 tỷ 2"; (4) ✅ đúng một câu "Dạ em sửa lại Phường 2 rồi ạ"; (1) ✅ "gia 7 ti 5 chu ko phai 7 ty 2" → `price_vnd` 7,5 tỷ, 🤖 "giá 7 tỷ 5", lời xác nhận "sửa lại giá 7 tỷ 5"; (3) ✅ `bathrooms` 2; (2) ✅ "em xoá cái hẻm 4m ghi nhầm đi" → "Dạ tin mình đang ghi hẻm 3.5m ạ, anh chị nhắn hẻm đúng là em sửa lại liền" + nhắc nhắn ok, `street` giữ "Trần Bình Trọng", hẻm 3.5, không thêm fact; (7) ✅ "😂😂" lúc duyệt → "Dạ 😊 Bản nháp ở trên…" (`loai_cau` emoji); (5) ✅ "bán rồi hả em?" → "Dạ chưa bán ạ, tin mình đang chờ thêm thông tin, chưa lên kệ; có khách chốt là em báo"; "đăng đi" → lên kệ 85/100. `bot_errors` chỉ 1 dòng `geocode-listings overpass` (dịch vụ ngoài). **Lộ một chỗ mới (chưa vá)**: câu xác nhận "đúng rồi em, Trần Bình Trọng. à mà phường 2 chứ không phải phường 4" → mảnh "Trần Bình Trọng. à mà" vào `bo_sung`, in ra bản nháp "📝 Thêm: Trần Bình Trọng. à mà" (OPEN-57) |
| TS-DUONG-04 | FR-212 Zalo thật sau deploy `chat-reply`: rao "bán nhà hẻm 4m pham the hien quận 8, 60m2, 5 tỷ" → bong bóng 🤖 in "Phạm Thế Hiển"; rao "…hẻm 4m pham the hier…" → bot hỏi "Đường mình là Phạm Thế Hiển phải không", trả lời "đúng rồi" → sửa, hỏi lại câu treo | như cột trước, `bot_errors` 0 dòng mới | 🟡 21/09 10:03 UTC (deploy #170, `net.http_post`, người thử `mau-duong-a/b`): tin A → "🤖 Đã lưu: Nhà phố bán · Phạm Thế Hiển, Quận 8 · 60m² · hẻm xe hơi 4m · giá 5 tỷ" ✅; tin B → 🤖 KHÔNG có địa chỉ, không hỏi ❌ — AI sửa "pham the hier" thành "Phạm Thế Hiển", kiểm bằng chứng bỏ vì đổi chữ cái, địa chỉ rơi trước khi tới từ điển. Vá: lấy trích dẫn làm địa chỉ (e2e DUONG-08, 423/423). **Bắn lại 10:14 UTC sau deploy #171** (`mau-duong-c`): tin B → "🤖 Đã lưu: … pham the hier, Quận 8 …" + "Đường mình là Phạm Thế Hiển phải không anh/chị?" ✅; "đúng rồi em" → "🤖 Đã lưu: vị trí cụ thể: Phạm Thế Hiển · 📦 Tin giờ: … Phạm Thế Hiển, Quận 8", `street`/`location_raw` = Phạm Thế Hiển, `duong_goi_y` = false, hỏi tiếp giá ✅. Còn lại: "anh/chị" có gạch chéo trong câu tiền định khi chưa biết cách gọi (TONE_RULES cấm model, câu mẫu vẫn dùng) — việc riêng |
| TS-XOAHET-01 | FR-210: gọi `admin_xoa_het_khach_va_ro_hang('XOA HET')` bằng chủ DB; gọi bằng khoá publishable; gọi với chữ sai | trả JSON số dòng, bảng khách/tin về 0, `projects`/`wards`/`ctvs`/`mau_cau`/`bot_prompts` giữ nguyên, `bot_health(xoa_het)` có dấu; anon → 401/42501; chữ sai → 22023 | ✅ 15/09: đã xoá 3 người bán · 2 khách · 4 tin · 33 tin nhắn · 5 hội thoại · 9 fact · 5 câu hỏi · 4 dòng bóng · 3 fact dự án · 3 quota; anon → HTTP 401; `projects` 1639, `wards` 168 giữ nguyên. Nút trên `/admin` chưa bấm thử bằng trình duyệt (`tsc` xanh). **🔴 22/09 chủ dự án bấm nút thật → "Không xoá được: DELETE requires a WHERE clause"**: role `authenticator` của Supabase nạp `safeupdate`, mọi `delete from t;` trần trong hàm bị chặn khi gọi qua PostgREST — lượt 15/09 xanh vì chạy bằng chủ DB qua MCP, không đi cửa người dùng bấm. Vá `20260922a_xoa_het_qua_api` (`where true` ở 21 câu xoá; `soat-migration` xanh, md5 161 hàm khớp). **Chưa bấm lại trên `/admin` sau vá** — bài kiểm này từ nay phải gọi qua REST bằng JWT admin mới tính đạt |
