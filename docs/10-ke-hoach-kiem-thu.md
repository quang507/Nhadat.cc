...
| TS-GIONG-02 | FR-180 (f): `bun run giong` (Haiku 4.5, model production) rồi `bun run giong:cham` (giám khảo Opus 5) trên máy có `ANTHROPIC_API_KEY` | TẦNG 1 ≥ 20/24 lượt đạt mọi luật máy; TẦNG 2 mỗi cờ ≥ 20/24; `errors.jsonl` 0 dòng; chạy `--lap 2` để thấy độ lệch giữa hai lần | ⏳ chưa chạy — sandbox không có khoá; chủ dự án gật chi phí (~0,4 đô/lượt đủ hai tầng) rồi chạy trên máy local |
