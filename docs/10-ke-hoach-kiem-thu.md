...
| TS-NHUNG-02 | Sau deploy `ask-seller`/`chat-reply`/`nudge`: gọi lại `ask-seller` dry_run với cùng tin; soi `bot_errors` 1 giờ sau | 200, `asked` có câu; không dòng "listing không tồn tại" mới | ⏳ |
