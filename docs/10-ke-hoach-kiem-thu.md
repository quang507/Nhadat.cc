...
| TS-NHAN-02 | FR-211 thật sau deploy `chat-reply`: rao "bán nhà hẻm 4m Nguyễn Trãi quận 5, 60m2, 6 tỷ 5, khu yên tĩnh gần chợ" bằng Zalo id thử; `select nhan from listings order by created_at desc limit 1`; web `/nha-dat?nhan=yen_tinh` | 💾 có "nhãn tìm kiếm: yên tĩnh, gần chợ"; cột `nhan = {yen_tinh,gan_cho}`; web lọc ra đúng tin | ⏳ chờ deploy |
