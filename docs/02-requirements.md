...
| FR-150 | **Thiếu dữ liệu thì suy giảm êm, không chết lặng**: (a) loại BĐS chưa rõ là giá trị THẬT `chua_ro` (không phải NULL ngầm) và việc đầu tiên bot làm với tin chưa rõ loại là **đi hỏi loại đó** (`required_facts.chua_ro → loai_bds`); trả lời xong `chat-reply` ghi `listings.property_type`, lượt sau `listing_missing_facts` tự chuyển sang bộ câu hỏi đúng loại — hệ thống tự lấp trường thiếu thay vì đứng im. (b) Điều kiện "đủ thông tin thì lên web" treo trên CHÍNH bảng `listings` (trigger `trg_listings_autopublish` + hàm chung `listings_try_publish`), không phụ thuộc thứ tự ghi trong code. (c) `price_vnd` tính lại cả khi UPDATE `price_raw`. (d) Bridge resolve được Zalo ID từ SĐT thì **ghi ngược** vào `sellers`/`ctvs`/`admins` qua `escalation-feed` action `ack` — người bán tự được nhận diện từ lần nhắn sau, không chờ ai điền tay | M | soát cấu trúc dữ liệu 26/08/2026 (4 chỗ gãy: view inner-join nullable, nhận diện một-khoá không đường lùi, auto-publish treo nhầm bảng, drip lọc theo trạng thái tiếng Anh đã bỏ) |
