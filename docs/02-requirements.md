...
| FR-158 | **Token Zalo OA tự làm mới.** Access token OA sống ĐÚNG 25 tiếng, mà từ trước tới nay nó nằm chết một chỗ trong Vault (`ZALO_OA_ACCESS_TOKEN`) — không dòng code hay cron nào đổi. Mỗi lần cấp tay, bot sống được một ngày rồi câm, và câm theo kiểu tệ nhất: `sendZalo()` trả `error != 0` → hàm trả `false` → edge function vẫn trả HTTP 200 → `bot_health_tick()` (chỉ soi mã HTTP, NFR-18) không thấy gì → khách ngồi chờ câu trả lời không bao giờ tới. Nay token SỐNG nằm ở bảng `bot_tokens` (RLS bật, không policy nào = chỉ service_role), cron `zalo-token-tick` 12 tiếng/nhịp gọi edge `zalo-token-refresh` đi đổi `refresh_token` lấy cặp mới; Vault chỉ còn là HẠT GIỐNG cho lần chạy đầu. Nhịp 12 tiếng chứ không 24: lỡ MỘT nhịp vẫn còn nhịp sau cứu. Mọi nơi đọc token phải đi qua `zaloToken()`, gọi thẳng `secretOf()` là đọc lại hạt giống đã hết hạn. **Zalo XOAY refresh_token** — đổi xong là chuỗi cũ chết, nên `bot_tokens` là bản duy nhất giữ chìa khoá còn dùng được và BẮT BUỘC nằm trong `scripts/sao-luu.mjs` | M | soát mã 27/08/2026 |
| FR-159 | **Ảnh Zalo phải về kho nhà mình.** `image_url` webhook đưa sang là link CDN TẠM của Zalo; bản cũ nhét thẳng chuỗi đó vào `messages.body` và `listing_facts.answer`. Vài tuần sau Zalo thu hồi, link trả 404, và mọi nơi trỏ tới nó hỏng cùng lúc: ảnh trên trang tin vỡ, bot gửi lại cho khách ra ảnh vỡ, CTV mở hội thoại cũ chỉ thấy ô trống — không lỗi nào nổ, dữ liệu chỉ lặng lẽ mục đi, và tới lúc phát hiện thì ảnh gốc đã không còn ở đâu để lấy lại. Nay `luuAnhVaoKho()` tải về rồi đẩy lên bucket `listing-photos/<mã tin>/…` đúng quy ước FR-148 — nên ảnh chính chủ gửi qua chat TỰ xuất hiện trên trang tin, khỏi thao tác nào thêm. Up hụt thì vẫn dùng link tạm (có còn hơn không) và ghi sổ FR-152 | M | soát mã 27/08/2026 |
| FR-160 | **Sự kiện OA ngoài text/image không được vứt.** `zalo-webhook` cũ có `if (!isText && !isImage) return;` nuốt sạch mọi thứ khác, làm mất hai thứ đáng giá nhất kênh OA cho không. **(a) `follow`** mang tham số `ref` của deep-link — `zaloLink()` nhét sẵn `?ref=#BDS-Q5-0115` vào mọi nút Chat Zalo, đó ĐÚNG là ngữ cảnh FR-13/14 "khách vừa bấm từ trang tin nào sang". Bỏ qua là bot chào trống không, bắt khách kể lại thứ họ vừa bấm. Nay dựng thành một câu viết từ phía KHÁCH rồi thả vào đúng đường ống nhận diện mã căn sẵn có (`CODE_RE`), khỏi viết nhánh chào riêng. **(b) `unfollow`** là tín hiệu tiêu cực rõ ràng nhất người dùng phát ra được; bỏ qua thì `nudge` vẫn đi hỏi thăm người đã bỏ theo dõi — phiền người ta và đốt lượt model vào một kênh đã đóng. Nay ghi một dòng `messages` sender `system` và HUỶ mọi `reminders` pending của người đó. Dòng `system` hiện trong ngữ cảnh model dưới nhãn "(ghi chú hệ thống)", không gộp vào nhãn "EM" như cũ — gộp là bot đọc được "[khách bỏ theo dõi]" như câu chính mình từng nhắn | S | soát mã 27/08/2026 |
| FR-161 | **Đọc phường viết tắt (`P4 Q5`).** Người Sài Gòn gõ "P4", "P.4", "p 4" nhiều hơn hẳn "Phường 4", mà mẫu phía người bán chỉ bắt `phường|phuong`. Câu rao viết tắt vào tới nơi là `ward = null` — mà `ward` nằm trong bộ ba bắt buộc của trigger auto-publish FR-139 (giá + diện tích + phường). Hệ quả: tin đó KHÔNG BAO GIỜ tự lên web, nằm `cho_thong_tin` vĩnh viễn, không lỗi nào để tra. Nay một hàm `wardNumberOf()` DÙNG CHUNG cho cả hai đầu (câu rao người bán ghi cột `listings.ward`, và mô tả khu vực người mua lọc kho) — trước đó hai đầu dùng hai mẫu khác nhau và chúng lệch nhau. Có ranh giới không-phải-chữ trước `p` để "shop 4"/"top 3" không thành phường, chặn chữ số sau để `P15` ra 15 chứ không phải 1, và chặn số ngoài 1–16 — cận trên lấy từ CHÍNH dữ liệu đang có (`listings` có tin ở Phường 1…16) chứ không từ số phường theo sách, vì đặt 15 là mọi câu rao ở P16 lại rơi về `ward = null`, đúng cái bẫy này chỉ dời sang phường khác. Ba nơi phải khớp: `wardNumberOf()`, `WARDS` ở `/quan-ly` (đã nâng 15→16), ô xổ ở `/admin/dang-tin` [giả định BA — chưa có nguồn gốc nào chốt danh sách phường Quận 5] | M | soát mã 27/08/2026 |
| FR-162 | **Trần theo NGUỒN GỌI.** Trần 100 tin/24h (FR-146) đếm theo `conversation_id`, mà conversation sinh từ `external_user_id` — chuỗi do NGƯỜI GỌI tự đặt và không ai kiểm. Đổi id mỗi request là bộ đếm về 0: trần cũ chặn đúng người nó không định chặn (khách thật nhắn nhiều) và không chặn được người nó định chặn. Trần toàn cục theo ngày (FR-151) giữ phần TIỀN nhưng là công tắc chung — kẻ xoay id đốt hết hạn mức rồi khách thật ăn 429 tới hết ngày. Nay thêm tầng giữa: bảng `rate_counters` + RPC `bump_rate(key, limit, window)` đếm theo IP, mặc định 300 lượt/24h (`DAILY_IP_MSG_CAP`). **Chỉ áp cho lượt KHÔNG mang service_role**: `zalo-webhook` gọi bằng service_role và mọi khách Zalo đi chung dải IP hạ tầng Supabase — đếm IP ở đó là chặn nhầm cả nhà, mà đường ấy `external_user_id` do Zalo cấp nên trần theo conversation mới đúng chỗ. Thiếu header IP thì KHÔNG chặn (lưới phụ, không được chặn nhầm khách thật) | S | soát mã 27/08/2026 |
| FR-163 | **Không có kênh thì không được ghi là đã gửi.** `nudge` cũ để `sent` nguyên "none" khi người nhận không có Zalo ID, rồi vẫn ghi một dòng `messages` sender `bot` và đóng reminder thành `sent` — không tin nào từng rời khỏi máy chủ. Ba thứ hỏng cùng lúc và không thứ nào tự lộ: sổ hội thoại có tin bot chưa hề tồn tại (CTV đọc, tưởng khách đã được nhắc, nên không nhắc nữa); lời nhắc đóng vĩnh viễn không nhịp cron nào cứu; vẫn tốn lượt model soạn câu không ai đọc. Nhánh `reengage` còn tự bịt miệng mình 5 ngày vì dòng `reengage` ảo chính là thứ bộ đếm chống-spam đi tìm. Nay chặn TRƯỚC khi gọi model, và chỉ ghi sổ + đóng reminder khi tin THẬT SỰ đã đi. Phân biệt hai loại hỏng: **không có Zalo ID = hỏng cơ cấu** → `cancelled` (giữ pending thì nhịp sau lại kéo đúng mấy dòng đó lên, mà truy vấn có `.limit(5)` — chúng chiếm chỗ vĩnh viễn của lời nhắc gửi được); **chưa có token OA = trục trặc tạm** → giữ pending | M | soát mã 27/08/2026 |
| FR-164 | **NMG đăng được tin CHO THUÊ.** `/quan-ly` gán cứng `deal: "ban"` lúc insert, nên nhà môi giới rao mặt bằng hay phòng trọ cho thuê thì tin vẫn vào kho dưới cờ BÁN. Hỏng theo dây chuyền, không lỗi nào nổ: tin nằm sai ở `/mua-ban` thay vì `/cho-thue`, và bộ lọc kho của bot (`.eq("deal", …)`) không bao giờ đưa nó cho khách đang tìm thuê — cách duy nhất để chữa là admin vào Table Editor sửa tay từng dòng. Nay có ô xổ xuống Bán / Cho thuê trong form đăng tin | M | soát mã 27/08/2026 |
| FR-165 | **Nhãn tiếng Việt cho `listing_facts`.** Khối "Đã xác minh với chủ nhà" trên trang tin in THẲNG khoá DB ra màn hình: khách đọc được `do_rong_hem`, `phap_ly`, `dien_tich_tim_tuong`. Tên cột rò ra mặt tiền — khó đọc, lộ cấu trúc dữ liệu, đi ngược tone "nói tiếng người" của docs/06. Nay `FACT_LABEL` trong `lib/format.ts` (24 khoá, khớp `required_facts.fact_key`). CỐ Ý không dùng chung `FACT_LABELS` bên bot: bản của bot là câu HỎI dài có ngoặc giải thích vì nó đi vào prompt, bản này là nhãn bảng thông số nên phải ngắn. Khoá chưa khai thì rơi về chính khoá — xấu nhưng không vỡ trang | S | soát mã 27/08/2026 |
| FR-166 | **Điều hướng sau đăng nhập theo danh sách trắng.** `/dang-nhap` cũ chỉ biết đúng một đích (`next === "quan-ly"` → `/quan-ly`, còn lại `/tai-khoan`), nên admin bấm "Đăng nhập" từ `/admin` bị ném về `/tai-khoan` rồi phải tự gõ lại `/admin` — mỗi lần đăng nhập. Nay có bảng `DICH_HOP_LE` (`quan-ly`, `admin`, `tai-khoan`, `yeu-thich`) và `/admin` truyền `?next=admin`. Vẫn là DANH SÁCH TRẮNG chứ không nhận thẳng `next` làm URL: `next` là chuỗi query string ai cũng sửa được, nhận bừa là mở đường open-redirect — gửi link `/dang-nhap?next=https://trang-lua-dao…` rồi người ta đăng nhập xong bị bắn sang đó với vẻ ngoài là nhadat.cc vừa chuyển trang | S | soát mã 27/08/2026 |
| FR-167 | **Trang `/admin` không được treo vô hạn.** `getUser().then(...)` trần, không `.catch()`: mạng chớp một cái, Supabase Auth trả 5xx, hay phiên hỏng — promise reject, `.then` không chạy, `role` nằm nguyên ở `"loading"`, màn hình đứng im mãi mãi. Không thông báo, không nút thử lại, không dấu vết ngoài một unhandled rejection; F5 ra y như vậy. Nay bọc `try/catch`, rơi về `"anon"` KÈM câu giải thích lỗi (an toàn hơn treo, và không cấp quyền nhầm cho ai — RLS phía DB mới là hàng rào thật) | S | soát mã 27/08/2026 |
| FR-168 | **Thứ tự tin đã xem / đã lưu phải là thứ tự của KHÁCH.** Hai chỗ cùng gốc "quên rằng DB không trả về theo thứ tự mình truyền vào". **(a) `/tai-khoan`** sắp bằng `codes.indexOf(a.code) - codes.indexOf(b.code)`; tin chỉ có ở phía DB (xem từ máy khác) cho `indexOf = -1`, mà -1 nhỏ hơn MỌI chỉ số hợp lệ — nên tin xem từ điện thoại tuần trước bị đẩy lên trên căn vừa mở hai phút trước trên chính máy này, càng nhiều thiết bị càng lộn. Nay xếp theo hạng: tin của máy này trước (theo thứ tự localStorage), tin thiết bị khác sau (giữ nguyên thứ tự `viewed_at`). **(b) `/yeu-thich`** gọi `.in("code", codes)` rồi hiển thị thẳng; không ORDER BY thì Postgres trả theo thứ tự nào tiện cho nó — hay gặp nhất là thứ tự tin được TẠO, tức trang "tin anh chị đã lưu" xếp theo ngày đăng của người bán chứ không theo lúc khách bấm tim. Nay sắp lại theo mảng localStorage, đảo chiều để căn vừa thích nằm trên cùng | S | soát mã 27/08/2026 |
