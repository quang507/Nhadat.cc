...
**AI bóc tách tin người bán có BỐN chế độ, công tắc `app_config.boc_tach_ai`** (`tat` · `bong` · `ghi` · **`chinh`** từ 21/09/2026, FR-208 f). `chinh` = ĐẢO TẦNG: chat-reply CHỜ lượt AI (qua kiểm bằng chứng `kiem-bang-chung.ts`) trước khi luật quyết — câu treo, fact kèm, cột lõi lúc tạo tin đều lấy của AI; luật tìm-chuỗi chỉ đỡ khi model hỏng hoặc cho khoá AI không có chỗ nói (`KHOA_FACT_AI_BIET`). Hệ quả cho người sửa luật: **một luật regex mới trong `khop-cau-tra-loi.ts` / `boc-cau-rao.ts` cho khoá nằm trong `KHOA_FACT_AI_BIET` sẽ KHÔNG chạy ở production** khi AI còn sống — muốn AI đọc ra thứ đó thì sửa LUAT trong `_shared/ai/boc-rao.ts` (prompt) hoặc thêm một VÍ DỤ MẪU ở `_shared/ai/vi-du-boc-rao.ts` (FR-208 g — ví dụ phải qua `bun bot/tests/vi-du-boc-rao.mjs`: dạy model thứ code sẽ bỏ là đỏ), và thêm ca ở `bot/tests/kiem-bang-chung.mjs`. Không train model; "học" ở đây là ví dụ trong prompt. Đổi công tắc ở Table Editor là có hiệu lực lượt kế, không cần deploy; e2e mô phỏng bằng `globalThis.__cauHinh.boc_tach_ai`.
